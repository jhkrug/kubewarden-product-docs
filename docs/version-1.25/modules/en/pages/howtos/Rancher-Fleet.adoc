...
= Managing {project-name} with {fleet-product-name}
:revdate: 2025-05-22
:page-revdate: {revdate}
:description: Managing {project-name} with Rancher {fleet-product-name}.
...
You can manage {short-project-name} Helm charts,
like other Helm charts, with {fleet-LINK}.
{fleet-product-name} uses Kubernetes CRDs
to define a GitOps approach to managing Kubernetes clusters.
It does this by defining {fleet-bundles-LINK}.
...
Using {fleet-product-name}, you can code the chart dependencies using
...
{fleet-product-name} is no exception.
...
the {fleet-product-name} configuration and then waiting until it's applied.
...
For an example of {fleet-product-name} bundle definitions see
